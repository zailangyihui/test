...
<script>
	...
	export default {
		...
		mounted: function() {
			if(common.getCookie('user') && common.getCookie('pswd')) {
				console.log(common.getCookie('user'))
				this.ruleForm2.accounts = common.getCookie('user');
				this.ruleForm2.password = common.getCookie('pswd');
				this.checked = true;
			}
		},
		methods: {
				...
				this.$refs[formName].validate((valid) => {
					...
					if(valid) {
						...
						this.$store.dispatch('axiosPost', { url: 'LOGIN', params: this.ruleForm2 }).then(res => {
							...
							if(data.code == 0) {
								if(me.checked) {
									common.setCookie('user', me.ruleForm2.accounts, 360); //保存帐号到cookie，有效期360天
									common.setCookie('pswd', me.ruleForm2.password, 360); //保存帐号到cookie，有效期360天
								} else {
									common.delCookie('user');
									common.delCookie('pswd');
								}
								//store.state.user.isLoginin = true
								console.log(res.data)
								common.setCookie('userinfo', JSON.stringify(res.data.data), 360);
								me.$router.push('/about.go');
							} else {
								this.$message({
						          	showClose: true,
